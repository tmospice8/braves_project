<template>
  <div class="shared-page">
    <h2>Pitcher Outcomes</h2>

    <!-- Player Header with Name and Averages -->
    <div v-if="selectedPitcherPlays.length" class="player-header">
      <div class="player-header-info">
        <img src="@/assets/headshot-icon.jpg" alt="Headshot" class="player-header-headshot" />
        <h3 class="player-header-name">{{ getPitcherName(selectedPitcherPlays) }}</h3>
      </div>
      ...
      <div class="results-count">
        {{ filteredPlays.length }} of {{ selectedPitcherPlays.length }} plays
      </div>
    </div>

    <!-- Shared Filter Section -->
    <div class="filters-section" v-if="selectedPitcherPlays.length">
          ...
          <div class="multi-select">
            ...
            <div v-if="showOutcomeDropdown" class="dropdown-options multi">
              <label v-for="outcome in uniqueValues('PLAY_OUTCOME')" :key="outcome" class="checkbox-option">
                <input type="checkbox" :value="outcome" v-model="selectedOutcomes" />
                {{ outcome }}
              </label>
            </div>
          </div>
      ...
      <div v-if="hasActiveFilters" class="filters-active-summary">
        ...
        <div class="active-filter-tags">
          ...
          <span v-for="(filter, key) in activeNumericFilters" :key="key" class="filter-tag">
...
<script src="./PitchersPage.js"></script>
